...
- Da tach spell-gate trong main-flow:
  - Phan cho phep/chan special-key khi `tempDisableKey` duoc dua vao `Core/Engine/EngineKeyHandleEventMainFlowSpellGate.inc`
  - Cum nay gom logic check tone/vowel overrides va gate `allowSpecialDespiteTempDisable` de doi chieu behavior de hon
...
### Split EngineCoreState into 3 sub-includes
- `EngineBuffers.inc`: HookState, TypingWord/KeyStates buffers, tempDisableKey
- `EngineVietnameseHelpers.inc`: isVietnameseWordFromTypingWord, isVietnameseFromCanonicalTelex
- `EngineSessionVars.inc`: session-scope static variables + function prototypes

### Split EngineCoreLogic into sub-includes
- `EngineInputKeyMacros.inc`: ProcessingChar table, IS_KEY_* macros, isSpecialKeyForCurrentInputType
- `EngineCoreState.inc`: static state globals, Vietnamese helpers, function prototypes
...
- `EngineMarkHandlingInsertW.inc`: insertW

### Split EngineSessionStateOps into 2 sub-includes
- `EngineSessionInsert.inc`: insertKey, insertState
- `EngineSessionPersist.inc`: saveWord, saveSpecialChar, restoreLastTypingState, startNewSession

### Split EngineCharacterTransform into 2 sub-includes
- `EngineCharacterLookup.inc`: checkCorrectVowel, getCharacterCode
- `EngineVowelOps.inc`: findAndCalculateVowel, removeMark, canHasEndConsonant
...
- `EngineMarkHandlingOldMark.inc`: handleOldMark + insertMark
- `EngineMarkHandlingInsertChars.inc`: insertD + insertAOE + insertW

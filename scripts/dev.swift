...
let projectPath = rootURL.appendingPathComponent("Apps/macOS/PHTV.xcodeproj").path
let scheme = "PHTV"
...
func swiftFormat(_ extraArguments: [String]) throws {
    ...
    try run("/usr/bin/xcrun", [
        ...
        "--recursive", "--parallel",
        rootURL.appendingPathComponent("Apps/macOS/PHTV").path,
        rootURL.appendingPathComponent("Apps/macOS/Tests").path,
    ])
}
...
do {
    ...
    case "metadata-check":
        ...
        let projectText = try String(
            contentsOf: rootURL.appendingPathComponent("Apps/macOS/PHTV.xcodeproj/project.pbxproj"),
            encoding: .utf8
        )
        ...
        for plist in ["Info.plist", "PHTV.entitlements", "PrivacyInfo.xcprivacy"] {
            try run("/usr/bin/plutil", [
                "-lint", rootURL.appendingPathComponent("Apps/macOS/PHTV/\(plist)").path,
            ])
        }
